...
for name, df in results.items():
    if 'metrics/mAP50(B)' not in df.columns:
        print(f"Skipping {name}: 'metrics/mAP50(B)' column not found.")
        continue

    epochs = np.arange(min(len(df['metrics/mAP50(B)']), 200))
    x_smooth, y_smooth = smooth_curve(epochs, df['metrics/mAP50(B)'][:len(epochs)])
...
plt.grid(True)
plt.savefig("smoothed_mAP50_comparison_models.png")
