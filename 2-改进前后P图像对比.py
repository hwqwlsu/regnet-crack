...
for name, df in results.items():
    if 'metrics/precision(B)' not in df.columns:
        print(f"Skipping {name}: 'metrics/precision(B)' column not found.")
        continue

    epochs = np.arange(min(len(df['metrics/precision(B)']), 100))
    x_smooth, y_smooth = smooth_curve(epochs, df['metrics/precision(B)'][:len(epochs)])
...
plt.grid(True)
plt.savefig("smoothed_precision_comparison_models.png")
